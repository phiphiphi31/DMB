...
from .backbone import Encoder,  Bottleneck
# , Decoder
...
class Decoder(nn.Module):
    ...
    def forward(self, r4, r3, r2, f, test_dist = None):
        ...
        if test_dist is not None:
            ...
            dist = F.interpolate(dist.unsqueeze(0), size=(m4.shape[-2], m4.shape[-1])) # [1, 1, m4, m4]
        """
        # position encoding
        r3_pe = r3 +  dist
...
class AMB(nn.Module):
    ...
    def segment(self, frame, keys, values, num_objects, max_obj, test_dist = None):
                ...
                if self.mode == 'two':
                    for t in range(1, 2):
                        ...
                        logits, ps = self.segment(frame=frame[idx, t:t + 1], keys=tmp_key, values=tmp_val,
                                                  ...
                                                  test_dist=test_dist[idx, t:t + 1])  # test_dist[b, 3, w, h]
                    ...
                    for t in range(1, T):
                        ...
                        if t - 1 == 0 or self.mode == 'mask':
                            ...
                            pred_mask = out[0, 1:num_object + 1]  # tensor [1, 384, 384]
