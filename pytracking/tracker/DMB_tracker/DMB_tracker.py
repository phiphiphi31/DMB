...
import pytracking.utils.processing_utils as prutils

from pytracking.bbox_fit import fit_bbox_to_mask
from pytracking.mask_to_disk import save_mask
from pytracking.tracker.DMB_tracker.AMB_models import AMB
import seaborn as sns
import matplotlib.pyplot as plt
import os
import cv2

class DMB_Segm(BaseTracker):
    def initialize_features(self):
        self.AMB_model = AMB()
        if torch.cuda.is_available():
            self.AMB_model.cuda()
        self.AMB_model.eval()  # turn-off BN
        for p in self.AMB_model.parameters():
            p.requires_grad = False

        pth_path = self.params.pth_path
        self.AMB_model.load_param(torch.load(pth_path))
        if not getattr(self, 'features_initialized', False):
            self.params.features_filter.initialize(extractor= self.AMB_model.Encoder_Q)
        self.features_initialized = True

    def init(self, image, state, video=None, init_mask=None):
        self.video = video
        self.sequence_name = video
        self.initialize(image, state=state, init_mask=init_mask)
    ...
    def initialize(self, image, state, init_mask=None, *args, **kwargs):
        ...
        if len(state) == 8:
            ...
            x_ = np.array(state[::2]) #4 个横坐标
            y_ = np.array(state[1::2]) # 4个纵坐标
        ...
        """
        train_x = tensorlist[27, 1024, 16, 16]
        """

        # Generate label function
        init_y = self.init_label_function(train_x)
        """
        init_y = tensorlist[27, 1, 16, 16]
        """
        ...
        self.pre_save_flag = False
        self.savesave_flag = False
    ...
    def update_vos(self, image):
        out = self.track(image)
        return self.vos_mask_result

    def update_got10K(self, image):
        out = self.track(image)
        return out
    ...
    def track(self, image):
        ...
        """
        self.save_flag = True
        print('uncertainty score is :' + str(self.uncert_score))
        score_show = s.cpu().numpy()
        sns.heatmap(score_show[0, 0, :], cmap='YlGnBu_r')
        print('frame num:' + str(self.frame_num))

        folder = os.path.join("/home/jaffe/Pictures/", self.video)
        if not os.path.exists(folder):
            os.mkdir(folder)
        plt.savefig(folder + "/" + str(self.frame_num) + ".png")
        plt.close()

        if self.uncert_score > 2.5:
            self.save_flag = True
            print('uncertainty score is :' + str(self.uncert_score))
            score_show = s.cpu().numpy()
            sns.heatmap(score_show[0, 0, :], cmap='YlGnBu_r')
            ...
            folder = os.path.join("/home/jaffe/Pictures/score/", self.video)
            if not os.path.exists(folder):
               os.mkdir(folder)
            plt.savefig(folder + "/" + str(self.frame_num) + ".png")
            plt.close()
        ...
        if uncert_score < self.params.tracking_uncertainty_thr: # tracking_uncertainty_thr = 3
            self.scores = np.append(self.scores, max_score)
            if self.scores.size > self.params.response_budget_sz: # response_budget_sz = 25
                self.scores = np.delete(self.scores, 0) # 移除最早的分数
        ...
        self.params.use_segmentation = true
        uncert_score = 0
        self.params.uncertainty_segment_thr
        self.params.uncertainty_segment_thr = 10
        """
        if self.segmentation_task or (
            self.params.use_segmentation and uncert_score < self.params.uncertainty_segment_thr): # uncertainty_segment_thr = 10
            pred_segm_region = self.segment_target(image, new_pos, self.target_sz)
            """
            image =
            new_pos =
            self.target_sz =
            pred_segm_region = list[8]： 8个float64数字： 代表坐标 
    ...
    def init_label_function(self, train_x):
        # Allocate label function tl  [250, 1, 16, 16]
        self.y = TensorList([x.new_zeros(self.params.sample_memory_size, 1, x.shape[2], x.shape[3]) for x in train_x])

        # Output sigma factor
        output_sigma_factor = self.fparams.attribute('output_sigma_factor') #0.25
        self.sigma = (
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        """
        init_patch_crop = np.array[384, 384, 3], f_= 1.5609
        """
        self.segmentation_task = False
        if init_mask is not None:
            ...
            """
            mask = np.array[352, 624] 以boundingbox 为mask
            """
        ...
        """
        init_mask_patch_np = np.array[384, 384] 周围有渐变
        patch_factor_init = 1.5609
        """
        # 构建模型
        ##############################################################################################
        ...
        """
        folder1 = "/home/jaffe/PycharmProjects/d3s_me/aaai/aaaimgs/" + self.video
        folder2 = "/home/jaffe/PycharmProjects/d3s_me/aaai/aaaimgs_bg/" + self.video
        ...
        mask_bg = 1 - init_mask_patch_np

        sns.heatmap(init_mask_patch_np,vmin=0, vmax=1)
        plt.savefig(folder1 + '/' + str(self.frame_num) + "_mask.png")
        plt.close()

        sns.heatmap(mask_bg, vmin=0, vmax=1)
        plt.savefig(folder2 + '/' + str(self.frame_num) + "_mask_bg.png")
        plt.close()
        """
        """
        self.STM_model = STM()
        if torch.cuda.is_available():
            self.STM_model.cuda()
        self.STM_model.eval()  # turn-off BN
        for p in self.STM_model.parameters():
            p.requires_grad = False

       # pth_path = '/home/jaffe/PycharmProjects/d3s_me/pytracking/networks/nobackbone/DAVIS16/recurrent8.pth.tar'
        pth_path = self.params.pth_path
     #   pth_path = '/home/jaffe/PycharmProjects/d3s_me/new_stm/recurrent1.pth.tar'
     #   print('Loading weights:', pth_path)
        self.STM_model.load_param(torch.load(pth_path))
        """
        ...
        init_mask_patch_AMB_bg_np = 1 - init_mask_patch_np
        init_mask_patch_bg = 1 - init_mask_patch

        #sns.heatmap(init_mask_patch_stm_bg_np)
        #plt.show()

        """
        init_patch = tensor[384, 384, 3]
        init_mask_patch = tensor[384, 384]
        """
        if self.params.use_gpu:
            init_patch = init_patch.to(self.params.device)
            init_mask_patch = init_mask_patch.to(self.params.device)
            init_mask_patch_bg = init_mask_patch_bg.to(self.params.device)

            if self.params.segm_use_dist:
                dist_map = dist_map.to(self.params.device)
                dist_map = torch.unsqueeze(dist_map, dim=0)
                test_dist_map = dist_map
            else:
                test_dist_map = None

        # reshape image for the feature extractor
        init_patch = torch.unsqueeze(init_patch, dim=0).permute(0, 3, 1, 2) # tensor[1, 3, 384, 384]
        init_mask_patch = torch.unsqueeze(torch.unsqueeze(init_mask_patch, dim=0), dim=0) # tensor[1, 1, 384, 384]
        init_mask_patch_bg = torch.unsqueeze(torch.unsqueeze(init_mask_patch_bg, dim=0), dim=0) # tensor[1, 1, 384, 384]

        init_mask_patch_k = torch.cat([init_mask_patch_bg, init_mask_patch], 1).contiguous() # [1, k, 384, 384]

        """
         init_patch = tensor[1, 3, 384, 384]
         init_mask_patch = tensor[1, 1, 384, 384]
        """
        if init_mask is None:
            ...
            while iters < 1:
                ...
                ###################################################################
                with torch.no_grad(): # m_input init_mask_patch_input_stm
                    prev_key, prev_value, _ = self.AMB_model.memorize(init_patch, init_mask_patch_k)
                    # segment
                    logit, ps = self.AMB_model.segment(init_patch, prev_key, prev_value, test_dist_map)

                    out = torch.softmax(logit, dim=1)  # tensor [1, k = 6, set_h, set_w]
                    mask_AMB = out[0, 1, :, :].cpu().numpy()
                    mask = mask_AMB.astype(np.float32)
                    #sns.heatmap(mask)
                    #plt.show()
                ###################################################################
                ...
                if self.params.save_mask:
                    ...
                    a = save_mask(None, mask, segm_crop_sz, bb, image.shape[1], image.shape[0],
                              self.params.masks_save_path, self.sequence_name, self.frame_name)
                    folder = os.path.join("/home/jaffe/PycharmProjects/d3s_me/aaai/segm/ori/img/", self.video)
                    if not os.path.exists(folder):
                        os.mkdir(folder)
                    save_image = cv2.cvtColor(self.image, cv2.COLOR_BGR2RGB)
                    img_name = os.path.join(folder, self.frame_name) +'.jpg'
                    cv2.imwrite(img_name, save_image)


              #  sns.heatmap(mask)
              #  plt.show()

                mask_gpu = torch.unsqueeze(torch.unsqueeze(torch.tensor(mask), dim=0), dim=0).to(self.params.device) #[1, 1, 384, 384]
                iters += 1
        else:
        ...
        self.mask_pixels = np.array([np.sum(mask)]) # np.array[1]: 7539.37

        self.pre_frame_input = init_patch
        self.pre_mask_input = torch.cat((1-mask_gpu, mask_gpu), 1) # [1, k, 384, 384]
        self.keys = []
        self.values = []
        a=1
    ...
    def segment_target(self, image, pos, sz):
        ...
        self.seg_frame_num = self.seg_frame_num + 1
        ...
        if self.params.segm_use_dist:
            ...
            test_dist_map = None
        with torch.no_grad():  # m_input init_mask_patch_input_stm
            this_key, this_value, _ = self.AMB_model.memorize(self.pre_frame_input, self.pre_mask_input)

            temp_keys = torch.cat(self.keys + [this_key], dim=1) # + [this_key]
            temp_values = torch.cat(self.values + [this_value], dim=1) # + [this_value]

            if self.pre_save_flag == True:
                temp_keys = torch.cat([] + [this_key], dim=1)  # + [this_key]
                temp_values = torch.cat([] + [this_value], dim=1)  # + [this_value]
                self.savesave_flag = True

            """
             if len(self.keys):
               temp_keys = torch.cat(self.keys , dim=1)
               temp_values = torch.cat(self.values , dim=1)
            else:
               temp_keys = torch.cat(self.keys + [this_key], dim=1)
               temp_values = torch.cat(self.values + [this_value], dim=1)
            """
            # segment
            logit, ps = self.AMB_model.segment(patch_gpu, temp_keys, temp_values, test_dist_map)
         ...
         #   folder1 = "/home/jaffe/Pictures/mask/" + self.video
          #  if not os.path.exists(folder1):
         #       os.mkdir(folder1)
         #   patch_rgb = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
         #   cv2.imwrite(folder1 + '/' + str(self.frame_num) + "_img.jpg", patch_rgb)

          #  sns.heatmap(mask)
         #   plt.savefig(folder1 + "/" + str(self.frame_num) + ".png")
        #    plt.close()
#
            mask_gpu = torch.unsqueeze(torch.unsqueeze(torch.tensor(mask), dim=0), dim=0).to(self.params.device)  # [1, 1, 384, 384]
            self.pre_frame_input = patch_gpu
            self.pre_mask_input = torch.cat((1 - mask_gpu, mask_gpu), 1).contiguous()  # [1, k, 384, 384]


            if (self.seg_frame_num - 1) % 5 == 0: # 5, 10
               self.keys.append(this_key)
               self.values.append(this_value)
        ...
        """
        folder = "/home/jaffe/PycharmProjects/d3s_me/aaai/butfly/" + self.video
        if not os.path.exists(folder):
            os.mkdir(folder)
        mask_save = ps[0, :, :].cpu().numpy()
        mask_save = (mask_save > self.params.segm_mask_thr)
        mask_save = 1- mask_save
        sns.heatmap(mask_save, vmin=0, vmax=1)  # 
        plt.savefig(folder + "/" + str(self.frame_num) + "_butfly_bg.png")
       # plt.show()
        plt.close()
        ...
        """
        folder = "/home/jaffe/PycharmProjects/d3s_me/aaai/" + self.video
        folder1 = "/home/jaffe/PycharmProjects/d3s_me/aaai/aaaimgs/" + self.video
        folder2 = "/home/jaffe/PycharmProjects/d3s_me/aaai/aaaimgs_bg/" + self.video
        ...
        patch_rgb = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
        mask_bg = 1 - mask
        cv2.imwrite(folder1 + '/' + str(self.frame_num) + "_img.png", patch_rgb)

        sns.heatmap(mask)
        plt.savefig(folder + '/' + str(self.frame_num) + "_mask.png")
        plt.close()

        sns.heatmap(mask_bg)
        plt.savefig(folder2 + '/' + str(self.frame_num) + "_mask.png")
        plt.close()
        ...
        if self.segmentation_task:
            ...
            if self.params.save_mask:
                self.vos_mask_result = save_mask(None, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0],
                          self.params.masks_save_path, self.sequence_name, self.frame_name)
           # return self.vos_mask_result
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  # 1000:
            ...
            if self.params.segm_optimize_polygon:
                if not self.segmentation_task:
                    ...
                    # Note: move this below if evaluating on VOT
                    if self.params.save_mask:
                        self.track_mask_result = save_mask(mask, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0],
                                  self.params.masks_save_path, self.sequence_name, self.frame_name)
            ...
            if not self.params.segm_scale_estimation or pixels_ratio < self.params.segm_pixels_ratio:
               ...
               if self.save_flag == True:
                    self.pre_save_flag = True

                if self.savesave_flag == True:
                   folder = os.path.join("/home/jaffe/Pictures/pic/", self.video)
                   if not os.path.exists(folder):
                       os.mkdir(folder)
                   save_image = cv2.cvtColor(self.image, cv2.COLOR_BGR2RGB)
                   draw_p = np.array([[pred_region[0], pred_region[1]],
                                   [pred_region[2], pred_region[3]],
                                   [pred_region[4], pred_region[5]],
                                   [pred_region[6], pred_region[7]]], np.int32)
                   imgplt = cv2.polylines(save_image, [draw_p], True, (0, 255, 255), thickness=2)
                   img_name = os.path.join(folder, str(self.frame_num)) + '.jpg'
                   cv2.imwrite(img_name, imgplt)
                   self.save_flag = False
                   self.pre_save_flag  = False
                   self.savesave_flag = False
                """

                return pred_region # list[8]
