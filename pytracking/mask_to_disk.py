...
def save_mask(mask, mask_real, segm_crop_sz, bb, img_w, img_h, masks_save_path, sequence_name, frame_name):
    ...
    image_mask[yi0:yi1, xi0:xi1] = mask_resized[yp0:yp1, xp0:xp1] # np.array[w, h]
   # sns.heatmap(image_mask)
   # plt.show()
   ...
   # cv2.imwrite(mask_save_path, image_mask)
    ...
    palette = Image.open('/home/jaffe/Titanspace/datasets/dataset/testdataset/dataset/Davis/DAVIS2017/Annotations/480p/blackswan/00000.png').getpalette()
    mask_label = Image.fromarray(image_mask)
    mask_label.putpalette(palette)
    mask_label.save(mask_save_path)
