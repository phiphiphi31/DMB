...
class ATOMResNet50(MultiFeatureBase):
    ...
    def initialize(self, extractor = None):
        ...
        self.layer_stride = {'conv1': 2, 'layer1': 4, 'layer2': 8, 'layer3': 16, 'layer3_01':16, 'layer4': 32, 'classification': 16, 'fc': None}
        self.layer_dim = {'conv1': 64, 'layer1': 256, 'layer2': 512, 'layer3': 1024, 'layer3_01':1024, 'layer4': 2048, 'classification': 256,'fc': None}
    ...
    def extract(self, im: torch.Tensor):
        ...
        if isinstance(output_features, dict):
            re = TensorList([output_features[layer] for layer in self.output_layers])
        elif isinstance(output_features, tuple):
            re = TensorList([output_features[0]])
        return re
